...
function tryFinish() {
	if (solved[0] && solved[1]) {
		...
		setTimeout(function(){
			...
			setTimeout(function(){$.post('http://ob/callback', JSON.stringify({'success': true, 'remainingtime': endTime-now}));}, 2000);
		}, 2000);
		resetGameState();
	}
}
...
function seedTable() {
	...
	if (now >= endTime) {
		...
		setTimeout(function(){
			...
			$.post('http://ob/callback', JSON.stringify({'success': false, 'remainingtime': 0}));
		}, 2000);
		resetGameState();
	} else {
		writeTime();
...
$(function(){
	...
	document.onkeydown = function(event) {
		if (gameStarted) {
			...
			if (charCode == 90 && !solved[0]) { // w
				if ( userSolPos[0][0] > 0 ) {
					userSolPos[0][0]--;
				}
			}
			else if (charCode == 81 && !solved[0]) { //a
				if ( userSolPos[0][1] > 1 ) {
					userSolPos[0][1]--;
				}
			}
			...
			if (charCode == 82) {
				...
				setTimeout(function(){
					fadeBigMsg(true);
					playSound('audiofail',1);
					$.post('http://ob/callback', JSON.stringify({'success': false, 'remainingtime': 0, 'surrend': true}));
				}, 2000);
				resetGameState();
				$('#hackDevice').fadeOut();
			}
		...
		if (item.hack) {
			if ('start' in item) {
				if('s' in item && 'd' in item) {
					setBigMsg('<span class="flash">Starting Hack...</span><br /><span class="small">Trouvez les blocs constants</span>');
