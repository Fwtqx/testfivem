GM = { State = { RadioState = 0, RadioID = {}, RadioFrequence = 0, SpectateMode = false } }
local itemBounds = { ["Radio"] = {20, 130}, ["Oreillette"] = {30, 120} }
local mainFrequence = 70.0
...
local function ToggleRadio()
	...
	if newMode == 0 then
		...
		if not IsConnectedToRoom("RADIO") then
			ConnectToRoom("RADIO", false)
		end
...
local function radioInit()
	...
	RegisterControlKey("openRadio", "Ouvrir la radio", "p", function()
		if exports.pphone2:IsPhoneOpen() then return end
...
Citizen.CreateThread(function()
	while true do
		Citizen.Wait(0)
		local useKeyboard = IsInputDisabled(2)

		if useKeyboard and GM.State.RadioState > 0 then
			if IsControlJustPressed(0, 182) and not IsDisabledControlPressed(1, 36) then
