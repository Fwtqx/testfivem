...
Citizen.CreateThread(function()
	while true do
		Citizen.Wait(0)
		if vehicle and DoesEntityExist(vehicle) and IsPedInVehicle(PlayerPedId(), vehicle) then
			local roll = GetEntityRoll(vehicle)
			if IsThisModelACar(GetEntityModel(vehicle)) and (roll > 75.0 or roll < -75.0) and GetEntitySpeed(vehicle) * 3.6 < 2 then
...
Citizen.CreateThread(function()
	while true do
		...
		if Player.InVehicle and veh and isPedDrivingAVehicle(ped, veh) and IsThisModelACar(GetEntityModel(veh)) and not IsControlPressed(1, 337) then
			vehicle = veh
