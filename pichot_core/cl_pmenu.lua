...
function PMenu:GetButtons(customMenu)
	...
	if not allButtons then self.tempData = { {}, 1 } return {} end

	local tblFilter = {}
	allButtons = type(allButtons) == "function" and allButtons(self, menu) or allButtons
	if not allButtons or type(allButtons) ~= "table" then self.tempData = { {}, 1 } return {} end
...
function PMenu:OpenMenu(stringName, boolBack, selectedButtonIndex)
	...
	selectedButtonIndex = selectedButtonIndex and selectedButtonIndex <= currentButtonsCount and selectedButtonIndex or 1

	-- Add in the history
	if self.Data.currentMenu ~= stringName and not boolBack and self.Data and self.Data.back then
		self.Data.back[#self.Data.back + 1] = { menu = self.Data.currentMenu, button = self.Pag[3] }
	end

	local max = math.max(10, math.min(selectedButtonIndex))
	self.Pag = { max - 9, max, selectedButtonIndex } -- min, max, current
...
function PMenu:BackMenu(tblBack)
	local menuName, previousButton = tblBack.menu, tblBack.button
	if menuName and not self.Menu[menuName] then print("[pMenu] " .. menuName .. " cannot be opened, the menu doesn't exist.") return end
	
	-- Delete the menu
	self.Data.back[#self.Data.back] = nil
	self:OpenMenu(menuName, true, previousButton)
...
function PMenu:Back()
	...
	elseif historyCount > 0 and not self.Base.BackBlocked then
		self:BackMenu(self.Data.back[#self.Data.back])
	end
end

function PMenu:CreateMenu(tableMenu, tempData)
	if (self.Base and self.Base.Blocked and self.IsVisible and exports.pichot_scaleform:IsMenuOpened()) or not tableMenu then return end
	if not self.IsVisible and tableMenu then
		...
		exports.pichot_scaleform:SetMenuVisible(boolVisible)
...
function PMenu:ProcessControl()
	...
	if (boolRIGHT or boolLEFT) and currentBtn and self:canUseButton(currentBtn) then
		...
		if currentBtn.parentSlider ~= nil and self:canUseButton(currentBtn) and ((boolLEFT and currentBtn.parentSlider < 1.5 + parentSliderSize) or (boolRIGHT and currentBtn.parentSlider > .5 - parentSliderSize)) then
			...
			if self.Events["onSlider"] then self.Events["onSlider"](self, self.Data, currentBtn, self.Pag[3], currentButtons, currentBtn.parentSlider - parentSliderSize) end
			Citizen.Wait(10)
		end
	end
...
if GetCurrentResourceName() == "pichot_core" then return end
