$(document).ready(function(){
	...
	function onSelected(data, forceClose) {
		$.post('http://42_beast/menuOnSelected', JSON.stringify([ data, forceClose ]));
	}
	...
	function CreateMenu(menuData, menuParams) {
		...
		for (let i = menuData.length - 1; i >= 0; i--) {
			...
			menus[i] = [ { name: 'Retour', icon: 'fas fa-sign-out-alt', cb: function() {
				metaMenu.hideMenu();
				if (i == 0) {
					onSelected();
	...
	document.onkeydown = function (data) {
		if (data.which && allowedKeys.includes(data.which)) {
			$.post('http://42_beast/menuKeyPress', JSON.stringify({ id : 1, key : data.which }));
		}
	};

	document.onkeyup = function (data) {
		if (data.which && allowedKeys.includes(data.which)) {
			$.post('http://42_beast/menuKeyPress', JSON.stringify({ id : 2, key : data.which }));
		}
	};
});